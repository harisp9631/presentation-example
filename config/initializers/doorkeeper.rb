...
Doorkeeper.configure do
  ...
  resource_owner_from_credentials do
    application = Doorkeeper::Application.by_uid_and_secret(
      params[:client_id],
  ...
  # falls back to the `:client_id` and `:client_secret` params
  ...
  # By default it retrieves first from the `HTTP_AUTHORIZATION` header, then
  # falls back to the `:access_token` or `:bearer_token` params
  # from the `params` object.
  # Check out the wiki for more information on customization
...
Doorkeeper::JWT.configure do
  ...
  # both secret_key and secret_key_path will be ignored
